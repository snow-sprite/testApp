...
// 已压缩文件数
let FINISHEDFILENUM: number = 0

// 要渲染的被压缩图片列表
let renderArr: IRenderArr[] = []
// {多文件类型}
ipcMain.on(
...
function readFPath(
  fPath: string,
  eventReply: Electron.IpcMainEvent,
  isSingle: boolean,
  type: string
) {
  fs.lstat(fPath, function(errs, stat) {
    ...
    if (stat.isFile()) {
      ...
      let minName = `${path.basename(
        realName,
        path.extname(realName)
      )}.min${path.extname(realName)}`
      ...
      // 重新生成的新名字
      let generatePathName: string = `${compressedTargetPath}${path.sep}${minName}`
      // tinypng api
      tinify
        .fromFile(path.resolve(fPath))
        .toFile(generatePathName, (errTiny, b) => {
          if (
            errTiny &&
          ...
          if (FILENUM === FINISHEDFILENUM) {
            ...
            let myZipper: any = zipper
            myZipper.zip(targetPath, (errZip: any, zipped: any) => {
              if (errZip) throw errZip
              zipped.compress()
              zipped.save(
                `${newTargetPath}${targetFileName}.zip`,
                (errSave: any) => {
                  if (errSave) throw errSave
                  console.log('last compressing..')
